...
type ReturnOutcome =
  | { outcome: "refunded"; returnId: number; refundedPln: number }
  | { outcome: "exchanged"; returnId: number; replacementSize: string }
  | { outcome: "repaired"; returnId: number; repairDays: number }
  | { outcome: "declined"; returnId: number; declineReason: string };

const describeReturnOutcome = (result: ReturnOutcome): string => {
  switch (result.outcome) {
    case "refunded":
      return `Zwrot ${result.returnId}: oddajemy ${result.refundedPln} zł`;
    case "exchanged":
      return `Zwrot ${result.returnId}: wysyłamy rozmiar ${result.replacementSize}`;
    case "repaired":
      return `Zwrot ${result.returnId}: naprawa potrwa ${result.repairDays} dni`;
    case "declined":
      return `Zwrot ${result.returnId}: odmowa, ${result.declineReason}`;
    default:
      const unknownResultOutcome: never = result;
      throw new Error(`Unknown result outcome: ${unknownResultOutcome}`);
  }
};

const returnOutcomes: ReturnOutcome[] = [
  { outcome: "refunded", returnId: 4101, refundedPln: 249 },
  { outcome: "exchanged", returnId: 4102, replacementSize: "M" },
  { outcome: "repaired", returnId: 4103, repairDays: 14 },
  { outcome: "declined", returnId: 4104, declineReason: "ślady użytkowania" },
...
const customerMessages = returnOutcomes.map((returnOutcome) =>
  describeReturnOutcome(returnOutcome),
);

customerMessages.forEach((customerMessage) => {
