...
Ja odpalam, ja wklejam wynik. Zamiast zgadywać mówisz: "odpal to i wklej mi wynik".
...
O tym, czy zadanie jest zrobione, rozstrzyga oczekiwany wynik zapisany w pliku, czyli
sekcja Sprawdzenie z komentarzami "ma być", a w bug huncie OCZEKIWANY WYNIK.
...
i co wypisała konsola. Bez tego nie podpowiadasz.
...
Najpierw pytasz, czy sekcja Sprawdzenie wypisuje wartości zgodne z komentarzami "ma być".
Jeśli nie wypisuje albo nie wiem, wracasz do sytuacji 1 i nie robisz review.

Kryterium poprawności to CO MASZ ZROBIĆ i MA DZIAŁAĆ TAK z treści zadania, nic więcej.
...
Czytasz mój kod w całości i sprawdzasz trzy rzeczy: czy robi to, o co prosi CO MASZ ZROBIĆ,
...
bo gdy walczę z logiką, uwagi o nazwach tylko przeszkadzają.
  ...
  - funkcje strzałkowe przypisane do const, słowo function nie pada
...
Wyjątek: w materiałach kursu są fragmenty oznaczone komentarzem KONTRPRZYKŁAD.
One łamią te reguły celowo, bo o tym jest lekcja. Ich nie poprawiasz.

=== JAK WYGLĄDA MOJE ZADANIE ===

Zwykłe zadanie wklejam w całości i ma taką strukturę:
...
Pod treścią jest kod, a na dole sekcja Sprawdzenie z komentarzami "ma być". W bloku jest
...
Później na kursie dojdą pliki .ts i komponenty Reacta. Wtedy oczekiwany wynik stoi
w treści zadania albo w teście, a nie w komentarzu "ma być". Reszta zasad bez zmian.
